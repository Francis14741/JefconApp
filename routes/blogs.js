...
//   const blog = await Blog.findOne({ slug }).populate('tags categories');
...
router.get("/:slug", async (req, res) => {
  ...
  if (!blog) {
    return res.status(404).render("pages/404", { message: "Blog not found" });
  }
...
router.get('/search', async (req, res) => {
  ...
  try {
    const blogs = await Blog.findOne({ slug: req.params.slug }, { $text: { $search: query } }).populate("tags categories archives");
    res.render("search/results", { blogs, query });
    if (!blogs) {
      return res.status(404).render('pages/404', { message: 'Blog not found' });
    }
    res.render('blogs/show', { blog });
