...
const blogsData = [
  ...
  },
  {
    ...
    tags: ["tag_drainagesystem", "tag_stormwater"],
    categories: ["uncategorized"],
    archive: ["archives_2020_may"],
    service: ["advisory_services"],
...
async function seedBlogs() {
  try {
    ...
    console.log("🗑️ Cleared old blogs");

    for (let blogData of blogsData) {
      // find referenced docs by slug
      const tags = await Tag.find({ slug: { $in: blogData.tags } });
      const categories = await Category.find({ slug: { $in: blogData.categories } });
      const archive = await Archive.findOne({ slug: blogData.archive });
      ...
      const blog = new Blog({
        ...
        archive: archive ? archive._id : null,
